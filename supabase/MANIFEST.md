...
| `send-booking-email` | **v10** | false | ✅ | Emails transaccionales (9 plantillas, Resend, idempotente, RGPD). **16-sep-2026 (v10)**: `holidu` como canal externo (los avisos del panel se saltan también a los huéspedes de Holidu). **15-sep-2026**: `booking_changed` y `booking_cancelled`, las dispara la madre desde el panel (un toque); **solo staff o clave de servicio** (403 si no), solo a quien reservó directo (canal Booking/Airbnb/… → `skipped: channel_booking`), nunca a un correo de relleno. `booking_changed` se repite (cada cambio su aviso); `booking_cancelled` exige `status=cancelled` y es idempotente. Lo que se devuelve lo lee de la nota que deja `cancel_booking` |
| `sync-ical-imports` | **v12** | false | ✅ | Importa calendarios iCal de canales (Airbnb/Booking/Escapada/CasasRurales.net/**Holidu**). **17-sep-2026 (v12) — un timeout suelto no es noticia**: «No se pudo leer el calendario» solo avisa si la pasada anterior (15 min antes) también falló; el 17-sep a las 11:30 Holidu tardó >20 s durante un minuto y salieron cuatro correos por nada. **16-sep-2026 (v11) — un cierre no es un overbooking**: un evento clasificado `closed` («Airbnb (Not available)», «CLOSED - Not available») ya no cuenta como conflicto contra una reserva propia; es lo que MisterPlan empuja al portal para reflejar la reserva que ya tenemos. Antes cantaba cada 15 min a Adrián (Booking) y a Michael (Holidu) «pisados» por su propio cierre (conflictos #4-#6, 5 correos falsos al buzón del negocio). Lo que trae huésped («Reserved», nombre, SUMMARY raro) sigue avisando. **15-sep-2026 (tarde) — canal Holidu** (mig. 0022): `holidu_ical_url` por apartamento, los cuatro conectados; sus reservas llegan con nombre abreviado («Michael S.») y se crean como reserva con `payment_method = ota` (lo cobra Holidu); un nombre completo ya apuntado no se pisa con la abreviatura. CasasRurales.net conectado en Tomillo (única ficha). **15-sep-2026 — reserva o cierre**: lee el `SUMMARY` («Reserved» vs «Airbnb (Not available)» / «CLOSED - Not available» sin NAME) y lo deja en `blocked_dates.external_kind` (mig. 0019), también en los bloqueos que ya existían (38 retipados en la primera pasada: 37 cierres, 1 reserva). El calendario de la madre enseña «Ocupado · por Airbnb» o «No alquilable · cerrado desde Airbnb» en vez de «Ocupado por otra web» para todo. **13-sep-2026 — reserva que se cancelaba sola**: daba por desaparecida del feed toda reserva cuyo UID no estuviera entre los eventos **con nombre**, y Airbnb nunca manda el nombre → las traídas de MisterPlan con UID de Airbnb se cancelaban solas en la primera pasada (José Luis, 220 €, 11-sep 19:15). Ahora mira el feed entero (`uidsEnFeed`), sincroniza las FECHAS de un evento sin nombre cuyo UID ya tiene reserva, y **avisa** cuando una reserva se cae (antes era silencioso). Prueba del parser dentro del runtime: `POST {"selftest":true}` |
...
| `issue-invoice` | **v4** | **true** | ✅ | **15-sep-2026 — una factura ANULADA no es «la factura de la reserva»**: `invoiceStateForBooking` calcula por cada original sus rectificativas, lo abonado y si está anulada (abonado ≥ total). `issue` ignora las anuladas (se puede emitir otra con número nuevo tras anular), `send` por reserva rechaza mandar una anulada (`invoice_anulada`, 409) y `status` devuelve `factura` (la viva), `abonado`, `anuladas` y `rectificativas`. **Toda la facturación, en una sola puerta** (10-sep-2026): emite (idempotente), genera el PDF y lo guarda en Storage, manda el correo con el PDF adjunto, hace rectificativas y firma URLs del PDF. Acciones: `issue`, `send`, `issue_and_send`, `rectify`, `pdf_url`, `status`. **YA NO usa la RPC `issue_invoice`**: la lógica fiscal vive en la propia función y su configuración (serie, IVA, emisor) en `functions/issue-invoice/config.ts`, único sitio |
...
- **Vistas**: `v_admin_kpis`, `v_admin_upcoming`, `v_admin_revenue_monthly`, `v_customers_360` (todas `security_invoker=true`; `bookings_email_queue` se retiró en la migración 0033, 18-sep) y **`v_parte_estado`** (10-sep; **14 columnas** desde la migración `0008`: las 12 de siempre más `completo` y `mandado_a_mano`) y **`v_channel_sync_status`** (10-sep, migración 0006, también `security_invoker=true`). Ojo: `v_admin_upcoming.precheckin_done`/`traveler_count` valen 0/false para `staff`, porque esa vista es invoker y `staff` no tiene política sobre `traveler_records`. Para el estado del parte, usar `v_parte_estado`.
- **Triggers de aplicación**: `guest_bookings_ensure_customer`, `guest_bookings_schedule_cleaning`, `guest_bookings_cleaning_follows` (11-sep), `booking_payments_recalc` y `guest_bookings_sync_stripe_payment` (10-sep), y varios `*_set_updated_at` (cleaning_tasks, invoices, kb_chunks, pricing_rules, protocols, traveler_records).
...
| `0016_la_llave_del_cron_se_lee_de_vault.sql` | aplicada | La llave `ses_cron_token` se lee de Vault desde la propia edge function (`tjm_llave_cron()`): se acaba el paso manual de copiarla al secreto `SES_CRON_TOKEN`, que llevaba desde el 10-sep sin darse (401 cada hora). Retira el cron duplicado de Trigger.dev |
| `0017_la_limpieza_sigue_a_la_reserva.sql` | aplicada | **La limpieza sigue a la reserva.** Medido el 11-sep probando el panel: al cambiar una reserva de fechas la tarea de limpieza se quedaba en la fecha vieja, y al cancelarla la limpieza pendiente sobrevivía. Trigger `guest_bookings_cleaning_follows` (`AFTER UPDATE OF status, check_out, apartment_id`): mueve la tarea PENDIENTE con la reserva y la borra si se cancela; las empezadas o hechas no se tocan. Lleva su prueba dentro |
| `0018_el_precheckin_dice_cuando_se_abre.sql` | aplicada | **El formulario del huésped dice cuándo se abre.** `tjm_precheckin_reserva` devolvía cero filas fuera de la ventana de siete días y el huésped leía «No encontramos esa reserva». Ahora devuelve una fila siempre que el código exista, con `ventana` (abierta / pronto / pasada / cancelada / sin_confirmar) y `check_in` para poder decir el día exacto; **fuera de la ventana, ningún dato personal** (nombre, correo, teléfono y apartamento van a NULL). Cambia la forma de lo que devuelve → `DROP` + `CREATE` + volver a dar el `GRANT` a `anon` |
| `0019_un_bloqueo_de_canal_dice_si_es_reserva_o_cierre.sql` | aplicada | **Un bloqueo de canal dice si es reserva o cierre.** Columna `blocked_dates.external_kind` (`reserved` / `closed`, NULL = propio o sin clasificar). La rellena `sync-ical-imports` v8 leyendo el `SUMMARY` del iCal; la lee `CalendarioPanel` (modo sencillo). Sin esto, un cierre que MisterPlan empuja a Airbnb y un huésped de Airbnb se enseñaban igual |
...
| `0025_informes_del_mes_por_apartamento_y_canal.sql` | aplicada | **Informes de Jesús (P6.3)**: `v_informe_mes(año, mes)` (filas total / apartamento / canal: noches disponibles y ocupadas, ocupación, entradas, cancelaciones, viajeros, pernoctaciones, brutos / comisión / netos prorrateados por noche, €/noche, estancia media) y `v_informe_serie(desde, hasta)` (totales mes a mes). SECURITY INVOKER, sin `anon`. La lee `InformesManager` (sustituye a `AnalyticsDashboard`, borrado). ⚠️ Los cierres de canal solo existen mientras el feed iCal los muestre: la disponibilidad de meses pasados se deforma con el tiempo (pendiente: foto mensual de cierres) |
