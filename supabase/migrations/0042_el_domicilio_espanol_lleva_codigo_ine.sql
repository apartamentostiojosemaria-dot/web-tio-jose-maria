-- 0042 — El domicilio español viaja con su código INE.
--
-- 19-sep-2026, 10:02: el Ministerio rechazó el primer parte de viajeros con
-- huéspedes españoles (lote 233e973c…, reserva TJM-F03114):
--   «Código de municipio obligatorio si el país es España».
-- El parte mandaba <nombreMunicipio>; el XSD lo admite, pero la validación
-- del Ministerio exige <codigoMunicipio> (código INE de 5 cifras) cuando el
-- país es ESP. Ningún parte con un español habría entrado nunca.
--
-- Además, el mismo caso destapó dos cosas más:
--   · nadie preguntaba al Ministerio cómo había quedado el lote: el parte se
--     quedó en «enviado · esperando validación» y el rechazo se supo por el
--     correo del MIR. Ahora hay una pasada de acuses (cron tjm-parte-acuses)
--     y el rechazo avisa al móvil y al buzón del negocio.
--   · los dos huéspedes volvieron a rellenar el formulario al día siguiente
--     y quedaron por duplicado (submit_traveler_records sólo sustituía fichas
--     SIN comunicar). Ahora una persona = una ficha por reserva, con índice.
--
-- Piezas:
--   1. ine_municipios: la relación oficial del INE (8.132 municipios a
--      1-ene-2026), con variantes de nombre para casar texto libre.
--   2. tjm_ine_municipio(nombre, cp): texto libre → código, o NULL si no hay
--      una única respuesta (nunca se inventa).
--   3. traveler_records.direccion_municipio_ine + trigger que lo rellena
--      solo, y relleno de las fichas que ya había.
--   4. tjm_municipios_buscar(q, cp): la lista para el formulario (anon).
--   5. tjm_fijar_municipio_viajero: arreglo desde el panel.
--   6. submit_traveler_records: guarda el código y no duplica personas.
--   7. Limpieza de los duplicados de TJM-F03114 + índice único.
--   8. Cron de acuses y permisos para que la función avise.
--
-- Fuente de los códigos: https://www.ine.es/daco/daco42/codmun/diccionario26.xlsx
-- (columna CPRO || CMUN; el dígito de control NO forma parte del código).
-- La misma lista está en supabase/data/ine_municipios_2026.csv; en producción
-- (20-sep-2026) las filas se cargaron desde ese CSV con pg_net porque el MCP
-- no traga 270 KB de INSERT. El resultado es idéntico a ejecutar este fichero.
-- Ojo: Hinojares es 23042. config.ts decía 23044, que es Huelma.

-- ---------------------------------------------------------------------------
-- 1. La relación de municipios
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.ine_municipios (
    codigo        char(5) PRIMARY KEY,
    provincia     char(2) GENERATED ALWAYS AS (left(codigo, 2)) STORED,
    nombre        text    NOT NULL,
    nombres_norm  text[]  NOT NULL DEFAULT '{}'
);
COMMENT ON TABLE public.ine_municipios IS
    'Relación de municipios del INE (1-ene-2026). codigo = provincia (2) + municipio (3), el que pide el MIR en <codigoMunicipio>. nombres_norm: variantes normalizadas para casar el texto del formulario.';

ALTER TABLE public.ine_municipios ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ine_municipios_lectura ON public.ine_municipios;
CREATE POLICY ine_municipios_lectura ON public.ine_municipios FOR SELECT TO anon, authenticated USING (true);
GRANT SELECT ON public.ine_municipios TO anon, authenticated;

-- Variantes de un nombre del INE, ya normalizadas (fn_ine_norm, 0007):
--   «Guardia de Jaén, La»  → GUARDIA DE JAEN, LA · LA GUARDIA DE JAEN · GUARDIA DE JAEN
--   «Agurain/Salvatierra»  → AGURAIN · SALVATIERRA (y la forma entera)
--   «Alfàs del Pi, l'»     → L'ALFAS DEL PI · ALFAS DEL PI
CREATE OR REPLACE FUNCTION public.fn_ine_variantes(p_nombre text)
 RETURNS text[]
...
DECLARE
    ...
    v_base  text;
BEGIN
    v_out := v_out || public.fn_ine_norm(p_nombre);
    FOREACH v_parte IN ARRAY string_to_array(p_nombre, '/') LOOP
        v_parte := btrim(v_parte);
        IF v_parte = '' THEN CONTINUE; END IF;
        v_out := v_out || public.fn_ine_norm(v_parte);
        -- «Nombre, Artículo» → «Artículo Nombre» y «Nombre» a secas.
        v_m := regexp_match(v_parte, '^(.*), ((?:la|el|los|las|l''|els|les|es|sa|os|as|a|o))$', 'i');
        IF v_m IS NOT NULL THEN
            v_base := v_m[1];
            v_art  := v_m[2];
            v_out := v_out || public.fn_ine_norm(v_base);
            IF right(v_art, 1) = '''' THEN
                v_out := v_out || public.fn_ine_norm(v_art || v_base);
            ELSE
                v_out := v_out || public.fn_ine_norm(v_art || ' ' || v_base);
            END IF;
        END IF;
    END LOOP;
    -- sin repetidos y sin vacíos
    SELECT COALESCE(array_agg(DISTINCT x), '{}') INTO v_out FROM unnest(v_out) x WHERE x <> '';
    RETURN v_out;
END $$;

-- Carga (idempotente): se rehace entera con la lista de este fichero.
INSERT INTO public.ine_municipios (codigo, nombre, nombres_norm)
SELECT v.codigo, v.nombre, public.fn_ine_variantes(v.nombre)
  FROM (VALUES
    ...
    ('12038','Castellfort'),
    ('12039','Castellnovo'),
    ('12040','Castelló de la Plana/Castellón de la Plana'),
    ...
    ('31069','Cáseda'),
    ('31070','Castejón'),
    ('31071','Castillonuevo/Gazteluberri'),
...
ON CONFLICT (codigo) DO UPDATE
   SET nombre = EXCLUDED.nombre, nombres_norm = EXCLUDED.nombres_norm;

CREATE INDEX IF NOT EXISTS ine_municipios_nombres_norm_gin ON public.ine_municipios USING gin (nombres_norm);
CREATE INDEX IF NOT EXISTS ine_municipios_provincia_idx ON public.ine_municipios (provincia);

-- ---------------------------------------------------------------------------
-- 2. Texto libre → código INE (o NULL: nunca se adivina)
-- ---------------------------------------------------------------------------
-- Orden de intentos, siempre exigiendo UNA sola respuesta:
--   a) nombre exacto dentro de la provincia del CP;
--   b) nombre exacto en toda España (por si el CP está mal);
--   c) empieza-por / contiene, dentro de la provincia del CP.
-- El nombre se limpia antes: se quita lo que va entre paréntesis
-- («Jaén (Jaén)»), los puntos y los espacios de más.
CREATE OR REPLACE FUNCTION public.tjm_ine_municipio(p_nombre text, p_cp text DEFAULT NULL)
 RETURNS char(5)
...
DECLARE
    ...
    v_num  int;
BEGIN
    v_n := public.fn_ine_norm(regexp_replace(COALESCE(p_nombre, ''), '\s*\([^)]*\)', '', 'g'));
    v_n := btrim(regexp_replace(regexp_replace(v_n, '[.]', '', 'g'), '\s+', ' ', 'g'));
    IF v_n = '' THEN RETURN NULL; END IF;

    v_prov := left(regexp_replace(COALESCE(p_cp, ''), '\D', '', 'g'), 2);
    IF v_prov IS NULL OR length(v_prov) < 2 OR v_prov NOT BETWEEN '01' AND '52' THEN v_prov := NULL; END IF;

    IF v_prov IS NOT NULL THEN
        SELECT count(*), min(codigo) INTO v_num, v_cod FROM public.ine_municipios
         WHERE provincia = v_prov AND nombres_norm @> ARRAY[v_n];
        IF v_num = 1 THEN RETURN v_cod; END IF;
    END IF;

    SELECT count(*), min(codigo) INTO v_num, v_cod FROM public.ine_municipios
     WHERE nombres_norm @> ARRAY[v_n];
    IF v_num = 1 THEN RETURN v_cod; END IF;

    IF v_prov IS NOT NULL AND length(v_n) >= 3 THEN
        SELECT count(*), min(codigo) INTO v_num, v_cod FROM public.ine_municipios m
         WHERE m.provincia = v_prov
           AND EXISTS (SELECT 1 FROM unnest(m.nombres_norm) x WHERE x LIKE v_n || '%' OR v_n LIKE x || '%');
        IF v_num = 1 THEN RETURN v_cod; END IF;
    END IF;

    RETURN NULL;
END $$;
COMMENT ON FUNCTION public.tjm_ine_municipio(text, text) IS
    'Municipio en texto libre (+ CP) → código INE de 5 cifras para el MIR. NULL si no hay una única coincidencia: entonces lo elige una persona.';
GRANT EXECUTE ON FUNCTION public.tjm_ine_municipio(text, text) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 3. La ficha del viajero guarda el código, y se rellena sola
-- ---------------------------------------------------------------------------
ALTER TABLE public.traveler_records
    ADD COLUMN IF NOT EXISTS direccion_municipio_ine char(5) REFERENCES public.ine_municipios(codigo);
COMMENT ON COLUMN public.traveler_records.direccion_municipio_ine IS
    'Código INE del municipio del domicilio (solo España). Lo pone el formulario si el huésped lo elige de la lista; si no, el trigger lo deduce del nombre + CP. NULL = no se pudo: el parte no sale hasta que alguien lo elija.';

CREATE OR REPLACE FUNCTION public.tjm_completar_municipio_ine()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'public', 'pg_temp'
AS $$
BEGIN
    IF NOT public.fn_ine_es_espana(NEW.direccion_pais) THEN
        NEW.direccion_municipio_ine := NULL;
        RETURN NEW;
    END IF;
    -- Cambia el nombre o el CP y nadie ha tocado el código en esta misma
    -- escritura: el código de antes ya no vale, se vuelve a deducir.
    IF TG_OP = 'UPDATE'
       AND NEW.direccion_municipio_ine IS NOT DISTINCT FROM OLD.direccion_municipio_ine
       AND (NEW.direccion_municipio IS DISTINCT FROM OLD.direccion_municipio
            OR NEW.direccion_cp IS DISTINCT FROM OLD.direccion_cp) THEN
        NEW.direccion_municipio_ine := NULL;
    END IF;
    IF NEW.direccion_municipio_ine IS NULL THEN
        NEW.direccion_municipio_ine := public.tjm_ine_municipio(NEW.direccion_municipio, NEW.direccion_cp);
    END IF;
    RETURN NEW;
END $$;

DROP TRIGGER IF EXISTS trg_completar_municipio_ine ON public.traveler_records;
CREATE TRIGGER trg_completar_municipio_ine
    BEFORE INSERT OR UPDATE OF direccion_municipio, direccion_cp, direccion_pais, direccion_municipio_ine
    ON public.traveler_records
    FOR EACH ROW EXECUTE FUNCTION public.tjm_completar_municipio_ine();

-- Las fichas que ya estaban.
UPDATE public.traveler_records t
   SET direccion_municipio_ine = public.tjm_ine_municipio(t.direccion_municipio, t.direccion_cp)
 WHERE t.direccion_municipio_ine IS NULL
   AND public.fn_ine_es_espana(t.direccion_pais);

-- ---------------------------------------------------------------------------
-- 4. La lista para el formulario (el huésped elige; el código va con la ficha)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.tjm_municipios_buscar(p_q text, p_cp text DEFAULT NULL)
 RETURNS TABLE(codigo char(5), nombre text, provincia text)
 LANGUAGE sql STABLE
 SET search_path TO 'public', 'pg_temp'
AS $$
    WITH q AS (
        SELECT btrim(regexp_replace(public.fn_ine_norm(p_q), '\s+', ' ', 'g')) AS n,
               NULLIF(left(regexp_replace(COALESCE(p_cp, ''), '\D', '', 'g'), 2), '') AS prov
    )
    SELECT m.codigo, m.nombre, public.fn_ine_provincia_por_cp(m.codigo)
      FROM public.ine_municipios m, q
     WHERE length(q.n) >= 2
       AND EXISTS (SELECT 1 FROM unnest(m.nombres_norm) x WHERE x LIKE q.n || '%' OR x LIKE '% ' || q.n || '%')
     ORDER BY (q.prov IS NOT NULL AND m.provincia = q.prov) DESC,
              (EXISTS (SELECT 1 FROM unnest(m.nombres_norm) x WHERE x LIKE q.n || '%')) DESC,
              length(m.nombre), m.nombre
     LIMIT 8;
$$;
COMMENT ON FUNCTION public.tjm_municipios_buscar(text, text) IS
    'Autocompletar del municipio en el formulario de la policía. Primero los de la provincia del CP, luego los que empiezan igual.';
GRANT EXECUTE ON FUNCTION public.tjm_municipios_buscar(text, text) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 5. Arreglo desde el panel (quien gestiona)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.tjm_fijar_municipio_viajero(p_traveler_id uuid, p_codigo char(5))
 RETURNS void
...
BEGIN
    ...
    END IF;
    SELECT nombre INTO v_nombre FROM public.ine_municipios WHERE codigo = p_codigo;
    IF v_nombre IS NULL THEN
        RAISE EXCEPTION 'municipio_desconocido' USING ERRCODE = '22023';
    END IF;
    UPDATE public.traveler_records
       SET direccion_municipio_ine = p_codigo,
           direccion_municipio     = v_nombre,
           updated_at              = now()
     WHERE id = p_traveler_id;
END $$;
REVOKE ALL ON FUNCTION public.tjm_fijar_municipio_viajero(uuid, char) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.tjm_fijar_municipio_viajero(uuid, char) TO authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 6. El formulario: guarda el código y no duplica a nadie
-- ---------------------------------------------------------------------------
-- Cambios respecto de 0014:
--   · acepta `direccion_municipio_ine` (lo que eligió el huésped en la lista);
--   · si la persona YA tiene ficha comunicada (submitted_at), se CORRIGE esa
--     ficha en vez de añadir otra. Antes sólo se retiraban las no
--     comunicadas, y una segunda entrega tras el envío duplicaba a todos.
...
BEGIN
    ...
    -- Las fichas SIN comunicar de las personas que vienen en esta entrega se
    -- retiran y se vuelven a crear (así cada uno corrige lo suyo). Las YA
    -- comunicadas se corrigen en el sitio, más abajo.
    ...
    FOR v_traveler IN SELECT * FROM jsonb_array_elements(p_travelers) LOOP
        SELECT tr.id INTO v_existente
          FROM public.traveler_records tr
         WHERE tr.booking_id = v_booking_id
           AND public.tjm_clave_viajero(tr.nombre, tr.apellido_primero, tr.fecha_nacimiento, tr.numero_documento)
             = public.tjm_clave_viajero(
                   v_traveler->>'nombre', v_traveler->>'apellido_primero',
                   NULLIF(v_traveler->>'fecha_nacimiento','')::date, v_traveler->>'numero_documento')
         ORDER BY tr.created_at
         LIMIT 1;

        IF v_existente IS NOT NULL THEN
            -- Misma persona, ficha ya comunicada: se corrigen los datos y se
            -- conserva lo del Ministerio (lote, estado, submitted_at).
            UPDATE public.traveler_records SET
                is_titular          = COALESCE((v_traveler->>'is_titular')::boolean, is_titular),
                apellido_primero    = v_traveler->>'apellido_primero',
                apellido_segundo    = v_traveler->>'apellido_segundo',
                nombre              = v_traveler->>'nombre',
                sexo                = v_traveler->>'sexo',
                tipo_documento      = v_traveler->>'tipo_documento',
                numero_documento    = v_traveler->>'numero_documento',
                soporte_documento   = v_traveler->>'soporte_documento',
                nacionalidad        = v_traveler->>'nacionalidad',
                fecha_nacimiento    = NULLIF(v_traveler->>'fecha_nacimiento','')::date,
                direccion_via       = v_traveler->>'direccion_via',
                direccion_municipio = v_traveler->>'direccion_municipio',
                direccion_municipio_ine = NULLIF(v_traveler->>'direccion_municipio_ine', ''),
                direccion_cp        = v_traveler->>'direccion_cp',
                direccion_pais      = v_traveler->>'direccion_pais',
                telefono_fijo       = v_traveler->>'telefono_fijo',
                telefono_movil      = v_traveler->>'telefono_movil',
                email               = v_traveler->>'email',
                parentesco          = NULLIF(upper(btrim(COALESCE(v_traveler->>'parentesco', ''))), ''),
                firma_base64        = COALESCE(v_traveler->>'firma_base64', firma_base64),
                updated_at          = now()
        ...
        ELSE
            INSERT INTO public.traveler_records (
                ...
                NULLIF(v_traveler->>'direccion_municipio_ine', ''),
    ...
    -- adulto responsable (mira a TODOS los adultos de la reserva).
    FOR v_nuevo IN
        SELECT tr.id
          FROM public.traveler_records tr
         WHERE tr.booking_id = v_booking_id
...
-- ---------------------------------------------------------------------------
-- 7. Duplicados que ya había + índice para que no vuelva a pasar
-- ---------------------------------------------------------------------------
-- TJM-F03114: las dos personas entraron el 18-sep y otra vez el 19-sep. Se
-- queda la primera ficha de cada una (la firmada primero). Si aparecieran
-- más duplicados en otra base, misma regla: se conserva la más antigua.
DELETE FROM public.traveler_records t
 USING public.traveler_records o
 WHERE o.booking_id = t.booking_id
   AND o.id <> t.id
   AND public.tjm_clave_viajero(o.nombre, o.apellido_primero, o.fecha_nacimiento, o.numero_documento)
     = public.tjm_clave_viajero(t.nombre, t.apellido_primero, t.fecha_nacimiento, t.numero_documento)
   AND (o.created_at < t.created_at OR (o.created_at = t.created_at AND o.id < t.id));

CREATE UNIQUE INDEX IF NOT EXISTS traveler_records_una_ficha_por_persona
    ON public.traveler_records (booking_id, public.tjm_clave_viajero(nombre, apellido_primero, fecha_nacimiento, numero_documento));

-- ---------------------------------------------------------------------------
-- 8. Los acuses se piden solos, y la función puede avisar al móvil
-- ---------------------------------------------------------------------------
-- La tanda de las 08:00 UTC manda; a las 09:15 UTC se pregunta al Ministerio
-- cómo quedó (lo procesa en minutos). La acción `acuses` mira partes,
-- reservas y anulaciones en «enviado_pendiente_acuse».
SELECT cron.unschedule('tjm-parte-acuses') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'tjm-parte-acuses');
SELECT cron.schedule('tjm-parte-acuses', '15 9 * * *', $cron$SELECT public.tjm_disparar_ses('acuses');$cron$);

-- El rechazo avisa al móvil (0041) desde la edge function, con la clave de servicio.
GRANT EXECUTE ON FUNCTION public.tjm_notificar_push(text, text, text) TO service_role;
